...
> 说明：原版 Clash（`Dreamacro/clash`）项目已基本停止维护；本文中的 “Clash 机场/Clash 客户端” 泛指规则分流生态的客户端/内核（常见为 `mihomo` / `sing-box` 系）。
...
### 了解自己的上网需求（先想清楚你要干什么）
- **视频观看**：需要高速、稳定的节点,且需要流媒体解锁能力强，建议选择IEPL或IPLC专线。
...
优质的 Clash 机场服务应在以下几个核心维度表现稳定可靠，你可以把它当作一份「体检清单」，逐项打分：

### 节点速度与稳定性
- 节点的带宽、运营商质量和线路类型（直连/中转/专线）几乎决定了真实体验；
- 建议选择有「中转节点」或「IEPL/专线」的服务，避免高峰期严重掉速；
- 重度用户可以优先关注是否标注「三网优化」「回程优化」「原生 IP」等信息。
...
### 月流量与价格性价比
- 不要只看「总流量」，要换算成「每 GB 单价」做横向对比，例如 1 元 ≈ 1G、10 元 ≈ 100G 等；
- 关注是否区分「国内回国流量 / 国际流量」，以及倍率设置（1x / 3x / 5x 等）；
- 对自己诚实：一个月实际会用多少流量？轻度用户没必要一上来就冲几 TB 年付。

### 是否有 IEPL / 优质专线加速
- IEPL/IPLC 专线不走公网，速度与稳定性远优于普通线路；
